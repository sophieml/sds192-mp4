...
ggplot(sequel_info %>% 
  ...
  summarize(n = n()) %>% 
  left_join(nall), aes(x = decade, y = n / numAll, fill = factor(link_type_id))) +
  geom_col(position = 'stack')
```

The figure above is a bar graph of sequels, remakes, and spinoffs as a proportion of all movies produced. If we only used this as criteria, claims about the proportions of sequels would be unfounded.

However, this approach is problematic because it disregards all other factors that influence human opinion. For example, the mean length between movies and their sequels decreases by a far larger factor than the proportion of sequels to original movies:
```{r}
sequel_info %>% 
  filter(!is.na(max_gross)) %>% 
  group_by(decade, title) %>% 
...
Of course, the amount of time is a confounding factor. However, the trend of decreasing time between sequels is still clear, even when limiting the amount of time between an original movie and its successors

<div class = "row">
<div class = "col-md-6">
```{r, fig.height=5, fig.width = 5, dev=c('svg')}
